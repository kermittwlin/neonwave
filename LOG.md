...
#### 資安修復
- [x] **移除硬編碼 YouTube API Key** — 避免在 GitHub 公開暴露，改為純使用者輸入模式
- [x] **新增 API Key 設定彈窗** — 首次使用時自動跳出，引導使用者自行設定
- [x] **重建倉庫** — 刪除舊 repo 重新上傳，確保 Git 歷史中無任何 API Key 殘留
...
```
83f0481  security: remove hardcoded YouTube API key, add UI modal for user input
4cc7f25  docs: add project log with full development history
